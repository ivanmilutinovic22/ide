...
### Feature 3 — Quick Panel Switch (s/w/t) + Number Item Selection
- `parsePaneShortcut`: s → sessions, w → windows, t → templates
- Border titles: `[s]-Sessions`, `[w]-Windows`, `[t]-Templates`
- Number keys 1–9 select items by index in the focused panel
...
### Feature 5 — Cursor Blink in Template Mode
- Both `openCreateTemplateMode()` and `startEditTemplateMode()` return `textinput.Blink`
- Catch-all in `Update()` routes blink ticks to the focused template textinput
